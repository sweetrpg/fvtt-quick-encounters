...
export const QE = {
    MODULE_NAME: "quick-encounters-2",
...
Hooks.on('closeJournalSheet', async (journalSheet, html) => {
    ...
    if (journalSheet.element.find("#QuickEncountersTutorial").length) {
        ...
        // 0.4.0 Check that we haven't already deleted this (because onDelete -> close)
        if (game.journal.get(journalEntry.id)) {
            // 0.8.3: Switch to use JournalEntry.deleteDocuments(ids)
            if (QuickEncounter.isFoundryV8Plus) {
                if (QuickEncounter.isFoundryV12Plus) {
    ...
    // 0.6.1: If there's a QE dialog open, close that too
    if (journalSheet.qeDialog) {
        journalSheet.qeDialog.close();
        delete journalSheet.qeDialog;
    }

    // 1.0.5e: Close open Journal Page Sheet QEs - for some reason the journalEntryPage.sheet is not updated so we have to use the getPageSheet() method
    // 1.0.7a: Check for isFoundryV10Plus
    if (QuickEncounter.isFoundryV10Plus) {
